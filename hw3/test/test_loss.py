...
def test_cross_entropy_loss(N, C):
    np.random.seed(42)
    ...
    expected_loss_numpy = cross_entropy_loss_np(input_np, ground_truth)

    output_gen = nn.cross_entropy(Tensor.from_numpy(input_np), Tensor.from_numpy(ground_truth))

    np.testing.assert_allclose(output_gen, expected_loss_numpy, rtol=1e-5, atol=1e-6)
